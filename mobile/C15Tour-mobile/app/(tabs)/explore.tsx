import { useState, useEffect, useRef, useCallback } from 'react';
import { Animated, Easing, Pressable, StyleSheet, Text, View, useWindowDimensions } from 'react-native';
import { WebView } from 'react-native-webview';
import MaterialIcons from '@expo/vector-icons/MaterialIcons';
import * as ScreenOrientation from 'expo-screen-orientation';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { useAppTheme } from '@/context/theme';
import { useAuth } from '@/context/auth';
import HomeButton from '@/components/ui/HomeButton';
import MicButton from '@/components/ui/MicButton';
import ConvoyName from '@/components/ui/ConvoyName';
import { API_BASE_URL } from '@/constants/api';
import MicIcon from '../../../../shared/global_assets/pictos/Mic.svg';
import MicMutedIcon from '../../../../shared/global_assets/pictos/MicMuted.svg';
import { getLocation, startHeadingTracking, startTracking, stopTracking } from '../services/locations/locationService';
import ScrollUpItinerary from '@/components/ui/scroll-up-itinerary';
import {
  computeDistanceToStart,
  computeGuidance,
  computeTripRoute,
  Coordinates,
  fetchTripSteps,
  GuidanceResult,
  reverseGeocodeStreetName,
  RouteGeometry,
  RouteSummary,
  TripStep
} from '../services/itinerary/ItineraryService';
import { checkAudioPermission } from '../services/permissions/microphonePermissionService';
import { LiveAudioSession, LiveAudioState } from '../services/audio/liveAudioService';


// Écran principal de l'application : carte en direct du convoi. Affiche une carte
// Leaflet (via une WebView chargeant du HTML/JS embarqué, faute d'équivalent React
// Native aussi complet que react-leaflet côté web) et communique avec elle par
// messages postMessage (voir mapHtmlContent plus bas et les webViewRef.current.postMessage
// disséminés dans le composant). Gère aussi le suivi GPS, l'envoi/réception de la
// position du "leader" (véhicule de tête), le guidage vers la prochaine étape et
// l'audio en direct type talkie-walkie (voir services/audio/liveAudioService.ts).

const MIC_STATUS_COLORS = {
  idle: '#CCCCCC',
  live: '#1DAD63',
  muted: '#D64545',
} as const;

const ITINERARY_CLOSED_RATIO = 0.15;
const ITINERARY_CONTROL_GAP = 50;
const STEP_REACHED_THRESHOLD_METERS = 50;
const AUDIO_STATUS_COLORS = {
  idle: '#CCCCCC',
  connecting: '#BB487C',
  waiting: '#BB487C',
  live: '#1DAD63',
  muted: '#D64545',
  paused: '#D99A28',
  unavailable: '#BB487C',
  unsupported: '#D64545',
  error: '#D64545',
} as const;

// Icône du véhicule de tête (leader), encodée en base64 pour être utilisable
// directement dans le HTML de la WebView sans dépendre d'un serveur d'assets
const LEADER_ICON_URL = 'data:image/svg+xml;base64,';

// Page HTML autonome chargée dans la WebView : carte Leaflet + tuiles OpenStreetMap,
// marqueurs (position de l'utilisateur, véhicule de tête, étapes), tracés de route,
// et un pont de communication avec React Native via window.postMessage (voir
// handleNativeMessage plus bas, qui reçoit les messages envoyés depuis le composant
// React via webViewRef.current.postMessage).
const mapHtmlContent = `
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.9.4/leaflet.min.css" />
    <script src="https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.9.4/leaflet.min.js"><\/script>
    <style>
        :root {
            --leaflet-control-left: 18px;
            --leaflet-control-bottom: 86px;
        }
        * { margin: 0; padding: 0; box-sizing: border-box; }
        html, body, #map { width: 100%; height: 100%; }
        .leaflet-control-container .leaflet-top.leaflet-left {
            top: auto;
            left: var(--leaflet-control-left);
            right: auto;
            bottom: var(--leaflet-control-bottom);
            transition: left 220ms ease, bottom 220ms ease;
        }
        .leaflet-control-container .leaflet-top.leaflet-left .leaflet-control {
            margin-top: 0;
            margin-left: 0;
        }
        .leader-leaflet-icon {
            background: transparent;
            border: none;
        }
        .leader-icon-img {
            width: 32px;
            height: 28px;
            transform-origin: center;
            filter: drop-shadow(0 2px 5px rgba(0, 0, 0, 0.35));
        }
        #leader-edge-indicator {
            position: absolute;
            z-index: 1000;
            width: 36px;
            height: 32px;
            display: none;
            pointer-events: none;
            transform: translate(-50%, -50%);
        }
        #leader-edge-indicator img {
            width: 100%;
            height: 100%;
            transform-origin: center;
            filter: drop-shadow(0 2px 5px rgba(0, 0, 0, 0.35));
        }
    </style>
</head>
<body>
    <div id="map"></div>
    <div id="leader-edge-indicator"><img src="${LEADER_ICON_URL}" alt="" /></div>
    <script>
        const map = L.map('map').setView([47.2165, -1.550], 13);
        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
            attribution: '© OpenStreetMap contributors',
            maxZoom: 19
        }).addTo(map);
        
        // Icônes personnalisées (encodées en SVG base64/data-URI) pour le véhicule
        // de l'utilisateur, le véhicule de tête (leader) et les points d'étape
        const vehicleIcon = L.icon({
            iconUrl: 'data:image/svg+xml;base64,',
            iconSize: [32, 28],
            iconAnchor: [16, 28],
            popupAnchor: [0, -28]
        });
        const leaderIcon = L.divIcon({
            html: '<img class="leader-icon-img" src="${LEADER_ICON_URL}" alt="" />',
            className: 'leader-leaflet-icon',
            iconSize: [32, 28],
            iconAnchor: [16, 28],
            popupAnchor: [0, -28]
        });
        
        const pinSvg = '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 12C15.0376 12 17.5 9.53757 17.5 6.5C17.5 3.46243 15.0376 1 12 1C8.96243 1 6.5 3.46243 6.5 6.5C6.5 9.53757 8.96243 12 12 12ZM12 12V23" stroke="#BB487C" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
        const pinSvgStartEnd = '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M13.155 0.0843026C9.1425 -0.541559 5.04413 2.39304 4.58995 6.37422C3.84251 12.9353 7.9087 15.7413 11.0308 23.2064C11.2275 23.6793 11.7281 24.0722 12.1394 23.9888C12.4326 23.9296 12.776 23.6063 12.9619 23.0534C15.7049 14.8546 19.417 14.4513 19.4993 7.82761C19.5458 3.97856 17.2785 0.72755 13.155 0.0843026ZM11.9069 20.5743L7.75134 13.3352C6.24217 10.8283 5.80587 7.94583 6.73927 5.17465C7.57968 3.24838 9.37138 2.10792 11.349 1.86106C13.4304 1.60376 15.5511 2.36522 16.8314 4.19413C18.3585 7.19827 17.9114 10.7066 16.0089 13.4743C14.4496 15.6891 13.248 17.8205 11.9034 20.5709L11.9069 20.5743Z" fill="#A53075"/><path d="M16.8351 4.19417C15.5548 2.36179 13.4341 1.60032 11.3527 1.8611C9.37506 2.10797 7.58336 3.24843 6.74295 5.17469C5.80955 7.94587 6.24585 10.8283 7.75502 13.3352L11.9106 20.5744C13.2553 17.8241 14.4533 15.6926 16.0161 13.4778C17.9187 10.7101 18.3657 7.20179 16.8387 4.19765L16.8351 4.19417ZM14.2852 9.88256C13.2016 11.2003 11.5244 11.5828 10.2906 11.0022C8.692 10.2511 8.36298 8.52305 9.02101 6.93753C9.80063 5.06343 11.8284 4.38541 13.4305 5.19903C15.2937 6.14825 15.7193 8.14058 14.2852 9.88604V9.88256Z" fill="#D1518B"/><path d="M13.4305 5.19903C11.8284 4.38193 9.79706 5.06343 9.02101 6.93753C8.36298 8.52305 8.692 10.2511 10.2906 11.0022C11.5244 11.5828 13.2016 11.2038 14.2852 9.88256C15.7193 8.1371 15.2936 6.14473 13.4304 5.19551L13.4305 5.19903ZM12.6794 9.02022C12.3361 9.42703 11.3991 9.85123 10.8591 9.30534C10.0544 8.48824 10.6803 6.96531 11.7138 6.58631C12.2467 6.3916 13.0335 6.80884 13.2731 7.39993C13.5735 8.15097 12.9727 8.67252 12.683 9.02022H12.6794Z" fill="#A53075"/></svg>';
        const pinIcon = L.icon({
          iconUrl: 'data:image/svg+xml;utf8,' + encodeURIComponent(pinSvg),
          iconSize: [30, 30],
          iconAnchor: [15, 29],
          popupAnchor: [0, -24]
        });
        const pinStartEndIcon = L.icon({
          iconUrl: 'data:image/svg+xml;utf8,' + encodeURIComponent(pinSvgStartEnd),
          iconSize: [30, 30],
          iconAnchor: [15, 29],
          popupAnchor: [0, -24]
        });

        const userMarker = L.marker([47.2165, -1.550], {
            icon: vehicleIcon,
            title: 'Ma position'
        }).addTo(map);

        const stepsLayer = L.layerGroup().addTo(map);
        const routeLine = L.polyline([], {
          color: '#BB487C',
          weight: 4,
          opacity: 0.9
        }).addTo(map);
        const nextStepLine = L.polyline([], {
          color: '#2F80ED',
          weight: 4,
          opacity: 0.95,
          dashArray: '8 10'
        }).addTo(map);
        let leaderMarker = null;
        let leaderAnimationFrame = null;
        let leaderLatLng = null;
        let leaderHeading = null;
        // Petite flèche affichée en bord d'écran pour indiquer la direction du
        // véhicule de tête quand celui-ci sort du cadre visible de la carte
        const leaderEdgeIndicator = document.getElementById('leader-edge-indicator');
        const leaderEdgeIcon = leaderEdgeIndicator.querySelector('img');

        // Ramène un cap (heading) quelconque dans l'intervalle [0, 360)
        const normalizeHeading = (heading) => {
            if (heading === null || heading === undefined || heading === '') return null;
            const numericHeading = Number(heading);
            if (!Number.isFinite(numericHeading)) return null;
            return ((numericHeading % 360) + 360) % 360;
        };

        const getLeaderIconElement = () => leaderMarker?.getElement()?.querySelector('.leader-icon-img');

        // Applique la rotation du cap à la fois sur l'icône du marqueur et sur l'indicateur de bord
        const applyLeaderHeading = () => {
            const rotation = normalizeHeading(leaderHeading) ?? 0;
            const markerIcon = getLeaderIconElement();
            if (markerIcon) markerIcon.style.transform = 'rotate(' + rotation + 'deg)';
            leaderEdgeIcon.style.transform = 'rotate(' + rotation + 'deg)';
        };

        // Affiche/masque l'indicateur de bord selon que le leader est visible ou
        // non dans la zone actuelle de la carte, et le positionne au bord le plus proche
        const updateLeaderEdgeIndicator = () => {
            if (!leaderLatLng) return;

            const bounds = map.getBounds();
            if (bounds.contains(leaderLatLng)) {
                leaderEdgeIndicator.style.display = 'none';
                return;
            }

            const size = map.getSize();
            const point = map.latLngToContainerPoint(leaderLatLng);
            const edgePadding = 20;
            const x = Math.min(Math.max(point.x, edgePadding), size.x - edgePadding);
            const y = Math.min(Math.max(point.y, edgePadding), size.y - edgePadding);

            leaderEdgeIndicator.style.display = 'block';
            leaderEdgeIndicator.style.left = x + 'px';
            leaderEdgeIndicator.style.top = y + 'px';
            applyLeaderHeading();
        };

        // Met à jour la position du marqueur du leader, en l'animant en douceur
        // (interpolation linéaire sur 900ms) plutôt qu'en le faisant "sauter"
        // directement à la nouvelle position reçue (la télémétrie n'arrive que
        // toutes les quelques secondes, cf. fetchLeaderPosition côté React).
        const updateLeaderMarker = (latitude, longitude, heading) => {
            leaderLatLng = L.latLng(latitude, longitude);
            leaderHeading = normalizeHeading(heading) ?? leaderHeading;

            if (!leaderMarker) {
                leaderMarker = L.marker(leaderLatLng, {
                    icon: leaderIcon,
                    title: 'Véhicule de tête'
                }).addTo(map);
                applyLeaderHeading();
                updateLeaderEdgeIndicator();
                return;
            }

            if (leaderAnimationFrame) {
                cancelAnimationFrame(leaderAnimationFrame);
            }

            applyLeaderHeading();
            updateLeaderEdgeIndicator();

            const start = leaderMarker.getLatLng();
            const end = L.latLng(latitude, longitude);
            const startTime = performance.now();
            const duration = 900;

            const animate = (now) => {
                const progress = Math.min((now - startTime) / duration, 1);
                const nextLat = start.lat + (end.lat - start.lat) * progress;
                const nextLng = start.lng + (end.lng - start.lng) * progress;

                leaderMarker.setLatLng([nextLat, nextLng]);

                if (progress < 1) {
                    leaderAnimationFrame = requestAnimationFrame(animate);
                } else {
                    leaderAnimationFrame = null;
                    updateLeaderEdgeIndicator();
                }
            };

            leaderAnimationFrame = requestAnimationFrame(animate);
        };

        // Point d'entrée unique de tous les messages envoyés depuis React Native
        // (webViewRef.current.postMessage) : un type de message par action possible sur la carte
        const handleNativeMessage = (event) => {
            try {
                const data = JSON.parse(event.data);
                if (data.type === 'SET_LOCATION') {
                    const { latitude, longitude, center = true } = data;
                    userMarker.setLatLng([latitude, longitude]);
                    if (center) {
                        map.setView([latitude, longitude], 13);
                    }
                    console.log('Map updated:', latitude, longitude);
                }
                if (data.type === 'SET_NEXT_STEP_LINE') {
                  const { coordinates } = data;
                  nextStepLine.setLatLngs([]);
                  if (Array.isArray(coordinates) && coordinates.length > 1) {
                    const latLngs = coordinates
                      .map((coord) => Array.isArray(coord) ? [coord[1], coord[0]] : null)
                      .filter((coord) => Array.isArray(coord));
                    if (latLngs.length > 1) {
                      nextStepLine.setLatLngs(latLngs);
                    }
                  }
                }
                if (data.type === 'SET_LEADER_LOCATION') {
                    const { latitude, longitude, heading } = data;
                    updateLeaderMarker(latitude, longitude, heading);
                    console.log('Leader updated:', latitude, longitude, heading);
                }
                if (data.type === 'SET_CONTROL_POSITION') {
                    document.documentElement.style.setProperty('--leaflet-control-left', data.left + 'px');
                    document.documentElement.style.setProperty('--leaflet-control-bottom', data.bottom + 'px');
                }
                if (data.type === 'SET_STEPS') {
                  const { steps } = data;
                  stepsLayer.clearLayers();
                  if (Array.isArray(steps) && steps.length > 0) {
                    const bounds = [];
                    steps.forEach((step, index) => {
                      if (typeof step.latitude !== 'number' || typeof step.longitude !== 'number') return;
                      const isEdge = index === 0 || index === steps.length - 1;
                      const marker = L.marker([step.latitude, step.longitude], {
                        icon: isEdge ? pinStartEndIcon : pinIcon
                      });
                      if (step.name) {
                        marker.bindPopup(step.name);
                      }
                      marker.addTo(stepsLayer);
                      bounds.push([step.latitude, step.longitude]);
                    });
                    if (bounds.length > 0) {
                      map.fitBounds(bounds, { padding: [24, 24] });
                    }
                  }
                }
                if (data.type === 'SET_ROUTE') {
                  const { coordinates } = data;
                  routeLine.setLatLngs([]);
                  if (Array.isArray(coordinates) && coordinates.length > 1) {
                    const latLngs = coordinates
                      .map((coord) => Array.isArray(coord) ? [coord[1], coord[0]] : null)
                      .filter((coord) => Array.isArray(coord));
                    if (latLngs.length > 1) {
                      routeLine.setLatLngs(latLngs);
                    }
                  }
                }
            } catch(e) {}
        };

        // react-native-webview délivre l'évènement "message" sur window (iOS) ou
        // sur document (Android) selon la plateforme : on écoute les deux par sécurité
        window.addEventListener('message', handleNativeMessage);
        window.addEventListener('resize', updateLeaderEdgeIndicator);
        document.addEventListener('message', handleNativeMessage);
        map.on('move zoom resize', updateLeaderEdgeIndicator);
    <\/script>
</body>
</html>
`;


type CallStatus = 'idle' | 'live' | 'muted';

export default function ExploreScreen() {
  const [isMicActive, setIsMicActive] = useState(false); // le panneau micro (leader) est-il déplié
  const [callStatus, setCallStatus] = useState<CallStatus>('idle'); // état de l'appel audio en tant que leader
  const [itineraryVisibleHeight, setItineraryVisibleHeight] = useState(0); // hauteur visible du panneau d'itinéraire (pour ne pas masquer les boutons flottants)
  const webViewRef = useRef<WebView>(null);
  const callStatusRef = useRef<CallStatus>('idle'); // miroir de callStatus lisible depuis des callbacks sans le recréer à chaque changement
  const leaderAudioSessionRef = useRef<LiveAudioSession | null>(null);
  const participantAudioSessionRef = useRef<LiveAudioSession | null>(null);
  const latestUserPositionRef = useRef<{ latitude: number; longitude: number } | null>(null);
  const { width, height } = useWindowDimensions();
  const insets = useSafeAreaInsets();
  const { colorScheme } = useAppTheme();
  const { trip, role } = useAuth();
  const isDark = colorScheme === 'dark';
  const tripId = trip?.trip_id;
  const isOrganizer = role === 'leader';
  const isLandscape = width > height;
  // Décalages calculés dynamiquement pour respecter les zones sécurisées (encoche,
  // barre de statut, gestes système) et adapter la disposition en paysage/portrait,
  // tout en évitant que les boutons flottants (recentrer, micro) ne chevauchent le
  // panneau d'itinéraire ou le panneau micro déplié.
  const topBarTop = isLandscape ? Math.max(16, insets.top + 8) : Math.max(40, insets.top + 8);
  const bottomSafeOffset = isLandscape ? Math.max(16, insets.bottom + 12) : Math.max(30, insets.bottom + 16);
  const horizontalSafeOffset = isLandscape ? Math.max(16, insets.right + 16) : 18;
  const leafletControlLeftOffset = isLandscape ? Math.max(16, insets.left + 16) : 18;
  const topBarHorizontalOffset = 15;
  const micButtonSafeOffset = isLandscape ? Math.max(0, insets.right + 16 - topBarHorizontalOffset) : 0;
  const panelHorizontalOffset = isLandscape
    ? Math.max(15, Math.max(insets.left, insets.right) + 16)
    : 15;
  const fallbackItineraryVisibleHeight = Math.ceil(height * ITINERARY_CLOSED_RATIO + insets.bottom);
  const effectiveItineraryVisibleHeight = itineraryVisibleHeight > 0
    ? itineraryVisibleHeight
    : fallbackItineraryVisibleHeight;
  const itineraryClearanceOffset = Math.ceil(effectiveItineraryVisibleHeight + ITINERARY_CONTROL_GAP);
  const micPanelBottomOffset = Math.max(bottomSafeOffset, itineraryClearanceOffset);
  const micPanelClearanceOffset = isMicActive
    ? micPanelBottomOffset + (isLandscape ? 120 : 190)
    : bottomSafeOffset;
  const recenterBottomOffset = Math.max(
    itineraryClearanceOffset,
    micPanelClearanceOffset
  );
  const leafletControlBottomOffset = recenterBottomOffset;
  const recenterBottomAnimated = useRef(new Animated.Value(recenterBottomOffset)).current;
  const [leaderAudioState, setLeaderAudioState] = useState<LiveAudioState>({
    status: 'idle',
    message: 'Micro inactif.',
  });
  const [leaderAudioError, setLeaderAudioError] = useState<string | null>(null);
  const [participantAudioState, setParticipantAudioState] = useState<LiveAudioState>({
    status: 'unavailable',
    message: 'Aucun audio disponible.',
  });
  const shouldRunLeaderAudio = isOrganizer && Boolean(tripId) && isMicActive && callStatus !== 'idle';

  // Répercute la position des contrôles Leaflet (zoom +/-) côté WebView pour
  // qu'ils restent alignés avec les boutons flottants React Native superposés
  const syncLeafletControlPosition = useCallback(() => {
    webViewRef.current?.postMessage(
      JSON.stringify({
        type: 'SET_CONTROL_POSITION',
        left: leafletControlLeftOffset,
        bottom: leafletControlBottomOffset,
      })
    );
  }, [leafletControlLeftOffset, leafletControlBottomOffset]);

  // Callback du panneau d'itinéraire : convertit sa position (en pixels depuis le
  // haut) en hauteur visible depuis le bas de l'écran, pour ajuster la position
  // des boutons flottants en conséquence
  const handleItinerarySheetChange = useCallback((_index: number, position?: number) => {
    if (typeof position !== 'number' || !Number.isFinite(position)) {
      setItineraryVisibleHeight(0);
      return;
    }

    setItineraryVisibleHeight(Math.max(0, height - position));
  }, [height]);

  // Anime en douceur le déplacement du bouton "recentrer" quand sa position cible change
  useEffect(() => {
    Animated.timing(recenterBottomAnimated, {
      toValue: recenterBottomOffset,
      duration: 220,
      easing: Easing.out(Easing.cubic),
      useNativeDriver: false,
    }).start();
  }, [recenterBottomAnimated, recenterBottomOffset]);

  // Réinitialise la hauteur connue du panneau d'itinéraire lors d'une rotation
  // d'écran ou d'un changement de zone sécurisée (la valeur précédente n'est plus valide)
  useEffect(() => {
    setItineraryVisibleHeight(0);
  }, [height, insets.bottom]);

  // Reverrouille l'orientation en portrait quand on quitte cet écran (au cas où
  // l'utilisateur l'aurait fait pivoter en paysage pendant la navigation)
  useEffect(() => {
    return () => {
      ScreenOrientation.lockAsync(ScreenOrientation.OrientationLock.PORTRAIT_UP);
    };
  }, []);

  useEffect(() => {
    syncLeafletControlPosition();
  }, [syncLeafletControlPosition]);

  // Garde une référence toujours à jour de callStatus, lisible depuis des
  // callbacks (ex: startLeader) sans provoquer leur recréation à chaque changement
  useEffect(() => {
    callStatusRef.current = callStatus;
  }, [callStatus]);

  // Coupe automatiquement le micro si l'utilisateur n'est plus organisateur
  // (ex: changement de convoi en cours de session)
  useEffect(() => {
    if (!isOrganizer) {
      setIsMicActive(false);
      setCallStatus('idle');
    }
  }, [isOrganizer]);

  // Démarre/arrête la session audio du leader selon les conditions requises
  // (organisateur, convoi actif, micro ouvert). Une nouvelle session WebRTC est
  // créée à chaque activation ; l'ancienne est toujours arrêtée proprement au nettoyage.
  useEffect(() => {
    if (!shouldRunLeaderAudio || !tripId) {
      leaderAudioSessionRef.current?.stop();
      leaderAudioSessionRef.current = null;
      setLeaderAudioState({
        status: 'idle',
        message: 'Micro inactif.',
      });
      return;
    }

    let isMounted = true;
    const session = new LiveAudioSession({
      tripId,
      role: 'leader',
      onStateChange: (state) => {
        if (isMounted) {
          setLeaderAudioState(state);
        }
      },
    });

    leaderAudioSessionRef.current = session;
    session.startLeader(callStatusRef.current === 'muted' ? 'muted' : 'live').catch((error) => {
      if (!isMounted) return;

      const message = error instanceof Error && error.message
        ? error.message
        : 'Impossible de lancer le direct audio.';
      setLeaderAudioError(message);
      setCallStatus('idle');
    });

    return () => {
      isMounted = false;
      session.stop();
      if (leaderAudioSessionRef.current === session) {
        leaderAudioSessionRef.current = null;
      }
    };
  }, [shouldRunLeaderAudio, tripId]);

  // Propage le changement d'état d'appel (live/muted) à la session en cours,
  // sans la recréer entièrement
  useEffect(() => {
    if (!shouldRunLeaderAudio) return;

    leaderAudioSessionRef.current?.setLeaderCallState(callStatus);
  }, [callStatus, shouldRunLeaderAudio]);

  // Démarre/arrête la session audio du participant : contrairement au leader,
  // elle est active en permanence dès qu'on est participant d'un convoi (pas de
  // bouton pour l'activer, seulement pour la mettre en pause côté écoute)
  useEffect(() => {
    if (role !== 'participant' || !tripId) {
      participantAudioSessionRef.current?.stop();
      participantAudioSessionRef.current = null;
      setParticipantAudioState({
        status: 'unavailable',
        message: 'Aucun audio disponible.',
      });
      return;
    }

    let isMounted = true;
    const session = new LiveAudioSession({
      tripId,
      role: 'participant',
      onStateChange: (state) => {
        if (isMounted) {
          setParticipantAudioState(state);
        }
      },
    });

    participantAudioSessionRef.current = session;
    session.startParticipant().catch((error) => {
      if (!isMounted) return;

      const message = error instanceof Error && error.message
        ? error.message
        : 'Connexion audio indisponible.';
      setParticipantAudioState({
        status: message.includes('WebRTC') ? 'unsupported' : 'error',
        message,
      });
    });

    return () => {
      isMounted = false;
      session.stop();
      if (participantAudioSessionRef.current === session) {
        participantAudioSessionRef.current = null;
      }
    };
  }, [role, tripId]);

  const [tripSteps, setTripSteps] = useState<TripStep[]>([]); // étapes du trajet, chargées depuis le backend
  const [routeGeometry, setRouteGeometry] = useState<RouteGeometry | null>(null); // tracé complet du trajet
  const [routeSummary, setRouteSummary] = useState<RouteSummary | null>(null); // distance/durée totales du trajet
  const [distanceToStartKm, setDistanceToStartKm] = useState<number | null>(null);
  const [currentLocation, setCurrentLocation] = useState<Coordinates | null>(null);
  const [currentStepIndex, setCurrentStepIndex] = useState(0); // prochaine étape à atteindre (progression dans le guidage)
  const [guidance, setGuidance] = useState<GuidanceResult | null>(null); // instruction de guidage vers la prochaine étape
  const [nextStepRouteGeometry, setNextStepRouteGeometry] = useState<RouteGeometry | null>(null); // tracé jusqu'à la prochaine étape (affiché en pointillés)
  const [userStreetName, setUserStreetName] = useState<string | null>(null);
  const [isMapReady, setIsMapReady] = useState(false); // la WebView a fini de charger la carte Leaflet
  const [networkError, setNetworkError] = useState(false); // échec de récupération de la position du leader (participant)
  const lastGeocodeLocationRef = useRef<Coordinates | null>(null);
  const geocodeMinMoveMeters = 50; // distance minimale avant de refaire un géocodage inverse (évite de spammer l'API)

  // Charge les étapes du trajet courant depuis le backend
  useEffect(() => {
    const fetchSteps = async () => {
      if (!trip?.trip_id) {
        setTripSteps([]);
        setRouteGeometry(null);
        setCurrentStepIndex(0);
        setNextStepRouteGeometry(null);
        return;
      }

      try {
        const data = await fetchTripSteps(trip.trip_id);
        setTripSteps(data);
        setCurrentStepIndex(0);
        setNextStepRouteGeometry(null);
      } catch (error) {
        console.error('Erreur reseau lors de la recuperation des etapes:', error);
        setTripSteps([]);
        setCurrentStepIndex(0);
        setNextStepRouteGeometry(null);
      }
    };

    fetchSteps();
  }, [trip?.trip_id]);

  // Calcule le tracé complet et le résumé (distance/durée) du trajet courant
  useEffect(() => {
    const fetchRoute = async () => {
      if (!trip?.trip_id) {
        setRouteGeometry(null);
        setRouteSummary(null);
        return;
      }

      try {
        const result = await computeTripRoute(trip.trip_id);
        setRouteGeometry(result.geometry);
        setRouteSummary(result.summary);
      } catch (error) {
        console.error('Erreur reseau lors du calcul de l\'itineraire:', error);
        setRouteGeometry(null);
        setRouteSummary(null);
      }
    };

    fetchRoute();
  }, [trip?.trip_id]);

  // Envoie les marqueurs d'étapes à la carte dès qu'elle est prête et à chaque changement
  useEffect(() => {
    if (!isMapReady || !webViewRef.current) return;

    const stepsPayload = tripSteps
      .map((step) => ({
        latitude: Number(step.step_latitude),
        longitude: Number(step.step_longitude),
        name: step.step_name ?? undefined,
      }))
      .filter((step) => Number.isFinite(step.latitude) && Number.isFinite(step.longitude));

    webViewRef.current.postMessage(
      JSON.stringify({
        type: 'SET_STEPS',
        steps: stepsPayload,
      })
    );
  }, [tripSteps, isMapReady]);

  // Envoie le tracé complet du trajet à la carte
  useEffect(() => {
    if (!isMapReady || !webViewRef.current) return;

    webViewRef.current.postMessage(
      JSON.stringify({
        type: 'SET_ROUTE',
        coordinates: routeGeometry?.coordinates ?? []
      })
    );
  }, [routeGeometry, isMapReady]);

  // Garde l'index de l'étape courante valide si la liste d'étapes change
  // (ex: étapes rechargées avec un nombre différent)
  useEffect(() => {
    if (tripSteps.length === 0) {
      setCurrentStepIndex(0);
    } else if (currentStepIndex >= tripSteps.length) {
      setCurrentStepIndex(tripSteps.length - 1);
    }
  }, [tripSteps, currentStepIndex]);

  // Envoie à la carte le tracé en pointillés jusqu'à la prochaine étape (guidage)
  useEffect(() => {
    if (!isMapReady || !webViewRef.current || !nextStepRouteGeometry) {
      webViewRef.current?.postMessage(
        JSON.stringify({
          type: 'SET_NEXT_STEP_LINE',
          coordinates: [],
        })
      );
      return;
    }

    webViewRef.current.postMessage(
      JSON.stringify({
        type: 'SET_NEXT_STEP_LINE',
        coordinates: nextStepRouteGeometry.coordinates,
      })
    );
  }, [nextStepRouteGeometry, isMapReady]);

  // Récupère le nom de la rue actuelle par géocodage inverse, mais seulement si
  // l'utilisateur s'est déplacé d'au moins geocodeMinMoveMeters depuis le dernier
  // appel (le géocodage inverse est coûteux, inutile de le refaire à chaque point GPS)
  useEffect(() => {
    const fetchStreetName = async () => {
      if (!currentLocation) {
        setUserStreetName(null);
        return;
      }

      const lastGeocodeLocation = lastGeocodeLocationRef.current;
      if (lastGeocodeLocation) {
        // Formule de Haversine pour calculer la distance parcourue depuis le dernier géocodage
        const toRadians = (value: number) => (value * Math.PI) / 180;
        const lat1 = toRadians(lastGeocodeLocation.latitude);
        const lat2 = toRadians(currentLocation.latitude);
        const deltaLat = toRadians(currentLocation.latitude - lastGeocodeLocation.latitude);
        const deltaLon = toRadians(currentLocation.longitude - lastGeocodeLocation.longitude);
        const a = Math.sin(deltaLat / 2) ** 2
          + Math.cos(lat1) * Math.cos(lat2) * Math.sin(deltaLon / 2) ** 2;
        const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
        const meters = 6371000 * c;
        if (meters < geocodeMinMoveMeters) {
          return;
        }
      }

      try {
        const street = await reverseGeocodeStreetName(currentLocation);
        setUserStreetName(street);
        lastGeocodeLocationRef.current = currentLocation;
      } catch (error) {
        console.error('Erreur reseau reverse geocode:', error);
        setUserStreetName(null);
      }
    };

    fetchStreetName();
  }, [currentLocation]);

  // Calcule la distance restante jusqu'au point de départ du trajet (pertinent
  // avant même d'avoir rejoint la première étape)
  useEffect(() => {
    const fetchDistanceToStart = async () => {
      try {
        const distance = await computeDistanceToStart(currentLocation, tripSteps);
        setDistanceToStartKm(distance);
      } catch (error) {
        console.error('Erreur reseau OSRM distance:', error);
        setDistanceToStartKm(null);
      }
    };

    fetchDistanceToStart();
  }, [currentLocation, tripSteps]);

  // Calcule l'instruction de guidage (distance/manœuvre) vers l'étape courante,
  // et passe automatiquement à l'étape suivante dès qu'on est suffisamment proche
  // (STEP_REACHED_THRESHOLD_METERS), sans action requise de l'utilisateur
  useEffect(() => {
    let isCancelled = false;

    const updateGuidance = async () => {
      if (!currentLocation || tripSteps.length === 0) {
        setGuidance(null);
        setNextStepRouteGeometry(null);
        return;
      }

      const target = tripSteps[currentStepIndex];
      if (!target) {
        setGuidance(null);
        setNextStepRouteGeometry(null);
        return;
      }

      const targetCoords = {
        latitude: Number(target.step_latitude),
        longitude: Number(target.step_longitude)
      };

      if (!Number.isFinite(targetCoords.latitude) || !Number.isFinite(targetCoords.longitude)) {
        setGuidance(null);
        setNextStepRouteGeometry(null);
        return;
      }

      try {
        const result = await computeGuidance(currentLocation, targetCoords);
        if (isCancelled) return;

        setGuidance(result);
        setNextStepRouteGeometry(result.geometry);

        if (typeof result.distanceToTargetMeters === 'number' && result.distanceToTargetMeters <= STEP_REACHED_THRESHOLD_METERS) {
          setCurrentStepIndex((prev) => Math.min(prev + 1, tripSteps.length - 1));
        }
      } catch (error) {
        if (isCancelled) return;

        console.error('Erreur reseau OSRM guidance:', error);
        setGuidance(null);
        setNextStepRouteGeometry(null);
      }
    };

    updateGuidance();

    return () => {
      isCancelled = true;
    };
  }, [currentLocation, tripSteps, currentStepIndex]);

  // Récupérer la position actuelle au chargement, puis démarrer le suivi GPS
  // continu. Si l'utilisateur est "leader", chaque nouvelle position (et le cap,
  // à raison d'au maximum une fois par seconde) est aussi envoyée au backend en
  // tant que télémétrie, pour que les participants puissent suivre sa position.
  useEffect(() => {
    let latestLeaderPosition: { latitude: number; longitude: number } | null = null;
    let lastHeadingSentAt = 0;

    const sendLeaderTelemetry = async (latitude: number, longitude: number, heading?: number | null) => {
      if (role !== 'leader' || !tripId) return;

      try {
        const normalizedHeading =
          typeof heading === 'number' && Number.isFinite(heading) && heading >= 0
            ? heading % 360
            : undefined;

        await fetch(`${API_BASE_URL}/api/trips/${tripId}/telemetry`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            latitude,
            longitude,
            heading: normalizedHeading,
            timestamp: new Date().toISOString(),
          }),
        });
      } catch (error) {
        console.error('Erreur lors de l\'envoi de la position leader:', error);
      }
    };

    const initializePosition = async () => {
      try {
        // Attendre 1.5 secondes pour laisser le GPS se stabiliser
        await new Promise(resolve => setTimeout(resolve, 1500));
        
        const location = await getLocation();
        if (location && webViewRef.current) {
          latestUserPositionRef.current = {
            latitude: location.coords.latitude,
            longitude: location.coords.longitude,
          };
          latestLeaderPosition = {
            latitude: location.coords.latitude,
            longitude: location.coords.longitude,
          };
          setCurrentLocation({
            latitude: location.coords.latitude,
            longitude: location.coords.longitude,
          });

          // Envoyer la position à la carte Leaflet
          webViewRef.current.postMessage(
            JSON.stringify({
              type: 'SET_LOCATION',
              latitude: location.coords.latitude,
              longitude: location.coords.longitude,
              center: true,
            })
          );
          await sendLeaderTelemetry(location.coords.latitude, location.coords.longitude, location.coords.heading);
        }

        // Démarrer le suivi continu pour mettre à jour la position en temps réel
        await startTracking((latitude, longitude, heading) => {
          latestUserPositionRef.current = { latitude, longitude };
          latestLeaderPosition = { latitude, longitude };

          setCurrentLocation({ latitude, longitude });
          if (webViewRef.current) {
            webViewRef.current.postMessage(
              JSON.stringify({
                type: 'SET_LOCATION',
                latitude,
                longitude,
                center: false,
              })
            );
          }
          sendLeaderTelemetry(latitude, longitude, heading);
        });

        if (role === 'leader' && tripId) {
          await startHeadingTracking((heading) => {
            if (!latestLeaderPosition) return;

            const now = Date.now();
            if (now - lastHeadingSentAt < 1000) return;
            lastHeadingSentAt = now;

            sendLeaderTelemetry(
              latestLeaderPosition.latitude,
              latestLeaderPosition.longitude,
              heading
            );
          });
        }
      } catch (error) {
        console.error('Erreur lors du positionnement du curseur:', error);
      }
    };

    initializePosition();

    // Nettoyage au déchargement
    return () => {
      stopTracking();
    };
  }, [role, tripId]);

  // Côté participant : interroge périodiquement (toutes les 5s) la dernière
  // position connue du leader et la transmet à la carte. Un 404 (pas encore de
  // position enregistrée) n'est pas traité comme une erreur réseau.
  useEffect(() => {
    if (role !== 'participant' || !tripId) {
      setNetworkError(false);
      return;
    }

    let isFetchingLeaderPosition = false;

    const fetchLeaderPosition = async () => {
      if (isFetchingLeaderPosition) return;
      isFetchingLeaderPosition = true;

      try {
        const response = await fetch(`${API_BASE_URL}/api/trips/${tripId}/telemetry/latest`);
        if (response.status === 404) {
          setNetworkError(false);
          return;
        }
        if (!response.ok) throw new Error(`HTTP ${response.status}`);

        const position = await response.json();
        if (!Number.isFinite(position.latitude) || !Number.isFinite(position.longitude)) return;

        setNetworkError(false);
        webViewRef.current?.postMessage(
          JSON.stringify({
            type: 'SET_LEADER_LOCATION',
            latitude: position.latitude,
            longitude: position.longitude,
            timestamp: position.timestamp,
            heading: position.heading,
          })
        );
      } catch (error) {
        setNetworkError(true);
        console.error('Erreur lors de la récupération de la position leader:', error);
      } finally {
        isFetchingLeaderPosition = false;
      }
    };

    fetchLeaderPosition();
    const intervalId = setInterval(fetchLeaderPosition, 5000);

    return () => clearInterval(intervalId);
  }, [role, tripId]);

  // Ouvre/ferme le panneau micro (leader) ; le fermer coupe aussi l'appel en cours
  const handleMicPress = () => {
    setIsMicActive((prev: boolean) => {
      const next = !prev;
      if (!next) {
        setCallStatus('idle');
        setLeaderAudioError(null);
        leaderAudioSessionRef.current?.stop();
        leaderAudioSessionRef.current = null;
      }
      return next;
    });
  };

  // Démarre l'appel (après vérification de la permission micro) ou bascule
  // entre "live" et "muted" s'il est déjà en cours
  const handleCallToggle = async () => {
    if (!tripId) {
      setLeaderAudioError('Aucun convoi actif.');
      return;
    }

    if (callStatus === 'idle') {
      const hasPermission = await checkAudioPermission();
      if (!hasPermission) {
        setLeaderAudioError('Permission micro refusee.');
        return;
      }

      setLeaderAudioError(null);
      setCallStatus('live');
      return;
    }

    setLeaderAudioError(null);
    setCallStatus((prev) => (prev === 'live' ? 'muted' : 'live'));
  };

  // Met en pause / reprend l'écoute de l'audio du leader côté participant
  const handleParticipantAudioToggle = () => {
    if (participantAudioState.status === 'live') {
      participantAudioSessionRef.current?.pauseParticipantAudio();
      return;
    }

    if (participantAudioState.status === 'paused') {
      participantAudioSessionRef.current?.resumeParticipantAudio();
    }
  };

  // Recentre la carte sur la position connue de l'utilisateur (ou la récupère à
  // la volée si elle n'a pas encore été capturée)
  const handleRecenterPosition = async () => {
    let position = latestUserPositionRef.current;

    if (!position) {
      const location = await getLocation();
      position = {
        latitude: location.coords.latitude,
        longitude: location.coords.longitude,
      };
      latestUserPositionRef.current = position;
    }

    webViewRef.current?.postMessage(
      JSON.stringify({
        type: 'SET_LOCATION',
        latitude: position.latitude,
        longitude: position.longitude,
        center: true,
      })
    );
  };

  // Passe manuellement à l'étape suivante (bouton "skip"), sans attendre d'être
  // physiquement arrivé à proximité de l'étape courante
  const handleSkipToNextStep = () => {
    setCurrentStepIndex((prev) => Math.min(prev + 1, Math.max(tripSteps.length - 1, 0)));
  };

  const statusContent = {
    idle: {
      status: 'MICRO INACTIF',
      label: "Appuie sur le micro pour lancer l'appel.",
    },
    live: {
      status: 'MICRO OUVERT',
      label: 'Les participants peuvent vous entendre.',
    },
    muted: {
      status: 'MICRO COUPE',
      label: 'On ne vous entend plus.',
    },
  }[callStatus];

  const RoundMicIcon = callStatus === 'muted' ? MicMutedIcon : MicIcon;
  const isCallLive = callStatus === 'live';
  const canSkipToNextStep = tripSteps.length > 1 && currentStepIndex < tripSteps.length - 1;
  const leaderPanelMessage = leaderAudioError
    ?? (leaderAudioState.status === 'connecting' ? leaderAudioState.message : statusContent.label);
  const participantAudioColor = AUDIO_STATUS_COLORS[participantAudioState.status];
  const participantAudioTitle = participantAudioState.status === 'live'
    ? 'AUDIO EN DIRECT'
    : participantAudioState.status === 'paused'
      ? 'AUDIO EN PAUSE'
      : 'AUDIO PARTICIPANT';
  const participantAudioIcon = participantAudioState.status === 'live'
    ? 'graphic-eq'
    : participantAudioState.status === 'paused'
      ? 'pause-circle-filled'
      : participantAudioState.status === 'muted'
        ? 'volume-off'
        : 'volume-up';
  const canToggleParticipantAudio = ['live', 'paused'].includes(participantAudioState.status);
  const participantAudioTop = topBarTop + (networkError ? 112 : 58);

  return (
    <View style={styles.container}>
      <WebView
        ref={webViewRef}
        source={{ html: mapHtmlContent }}
        style={styles.webview}
        javaScriptEnabled
        domStorageEnabled
        onLoadEnd={() => { setIsMapReady(true); syncLeafletControlPosition(); }}
      />

      <View
        style={[
          styles.topBar,
          {
            top: topBarTop,
            left: topBarHorizontalOffset,
            right: topBarHorizontalOffset,
          },
        ]}>
        <View style={styles.topBarSide}>
          <HomeButton />
        </View>

        <View style={styles.topBarCenter}>
          <ConvoyName />
        </View>

        <View style={[styles.topBarSide, { transform: [{ translateX: -micButtonSafeOffset }] }]}>
          {isOrganizer && <MicButton isActive={isMicActive} onPress={handleMicPress} />}
        </View>
      </View>

      {role === 'participant' && networkError && (
        <View
          style={[
            styles.networkBanner,
            {
              top: topBarTop + 58,
              left: panelHorizontalOffset,
              right: panelHorizontalOffset,
              backgroundColor: isDark ? 'rgba(28,28,30,0.96)' : 'rgba(255,255,255,0.96)',
            },
          ]}>
          <MaterialIcons name="wifi-off" size={18} color="#BB487C" />
          <Text style={styles.networkBannerText}>
            Connexion interrompue.
            Reconnexion automatique en cours...
          </Text>
        </View>
      )}

      {role === 'participant' && (
        <View
          style={[
            styles.audioBanner,
            {
              top: participantAudioTop,
              left: panelHorizontalOffset,
              right: panelHorizontalOffset,
              borderColor: participantAudioColor,
              backgroundColor: isDark ? 'rgba(28,28,30,0.96)' : 'rgba(255,255,255,0.96)',
            },
          ]}>
          <MaterialIcons name={participantAudioIcon} size={20} color={participantAudioColor} />
          <View style={styles.audioTextColumn}>
            <Text style={[styles.audioBannerTitle, { color: participantAudioColor }]}>
              {participantAudioTitle}
            </Text>
            <Text style={[styles.audioBannerText, { color: isDark ? '#FFFFFF' : '#3C3440' }]}>
              {participantAudioState.message}
            </Text>
          </View>
          {canToggleParticipantAudio && (
            <Pressable
              accessibilityRole="button"
              accessibilityLabel={participantAudioState.status === 'live' ? 'Mettre audio en pause' : 'Reprendre audio'}
              onPress={handleParticipantAudioToggle}
              style={({ pressed }) => [
                styles.audioActionButton,
                { borderColor: participantAudioColor },
                pressed && styles.audioActionButtonPressed,
              ]}>
              <MaterialIcons
                name={participantAudioState.status === 'live' ? 'pause' : 'play-arrow'}
                size={20}
                color={participantAudioColor}
              />
            </Pressable>
          )}
        </View>
      )}

      <ScrollUpItinerary
        speedKmh={trip?.trip_speed}
        distanceKm={routeSummary?.distanceKm}
        durationSeconds={routeSummary?.durationSeconds}
        distanceToStartKm={distanceToStartKm}
        distanceToNextTargetMeters={guidance?.distanceToTargetMeters ?? null}
        distanceToNextManeuverMeters={guidance?.distanceToNextManeuverMeters ?? null}
        nextInstruction={guidance?.instruction ?? null}
        streetName={userStreetName}
        onSheetChange={handleItinerarySheetChange}
      />


      <Animated.View
        style={[
          styles.recenterButtonPosition,
          { bottom: recenterBottomAnimated, right: horizontalSafeOffset },
        ]}>
        {canSkipToNextStep && (
          <Pressable
            accessibilityRole="button"
            accessibilityLabel="Passer a l'etape suivante"
            style={({ pressed }) => [
              styles.mapControlButton,
              pressed && styles.recenterButtonPressed,
            ]}
            onPress={handleSkipToNextStep}>
            <MaterialIcons name="skip-next" size={24} color="#2F80ED" />
          </Pressable>
        )}
        <Pressable
          accessibilityRole="button"
          accessibilityLabel="Recentrer ma position"
          style={({ pressed }) => [
            styles.mapControlButton,
            pressed && styles.recenterButtonPressed,
          ]}
          onPress={handleRecenterPosition}>
          <MaterialIcons name="my-location" size={24} color="#BB487C" />
        </Pressable>
      </Animated.View>

      {isOrganizer && isMicActive && (
        <View
          style={[
            styles.micPanel,
            {
              bottom: micPanelBottomOffset,
              left: panelHorizontalOffset,
              right: panelHorizontalOffset,
              backgroundColor: isDark ? 'rgba(28,28,30,0.96)' : 'rgba(255,255,255,0.96)',
            },
          ]}>
          <View
            style={[
              styles.roundMicButtonOuter,
              { borderColor: MIC_STATUS_COLORS[callStatus] },
            ]}>
            <Pressable
              style={[styles.roundMicButton, isCallLive && styles.roundMicButtonActive]}
              onPress={handleCallToggle}>
              <RoundMicIcon
                width={28}
                height={28}
                color={isCallLive ? '#FFFFFF' : '#BB487C'}
              />
            </Pressable>
          </View>

          <View style={styles.statusColumn}>
            <View style={styles.statusRow}>
              <Text style={styles.statusMic}>{statusContent.status}</Text>
            </View>
            <Text style={styles.confirmationText}>{leaderPanelMessage}</Text>
          </View>
        </View>
      )}
    </View>
  );
}

const styles = StyleSheet.create({
  container: {
    flex: 1,
  },
  webview: {
    flex: 1,
  },
  cursorVehicule: {
    position: 'absolute',
    top: '50%',
    left: '50%',
    transform: [{ translateX: -20 }, { translateY: -20 }],
  },
  topBar: {
    position: 'absolute',
    top: 40,
    left: 15,
    right: 15,
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    zIndex: 10,
  },
  topBarSide: {
    width: 44,
    alignItems: 'center',
    justifyContent: 'center',
  },
  topBarCenter: {
    flex: 1,
    alignItems: 'center',
    justifyContent: 'center',
    paddingHorizontal: 12,
  },
  networkBanner: {
    position: 'absolute',
    zIndex: 11,
    minHeight: 44,
    borderRadius: 8,
    borderWidth: 2,
    borderColor: '#BB487C',
    paddingHorizontal: 12,
    paddingVertical: 10,
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    gap: 8,
    elevation: 4,
  },
  networkBannerText: {
    flexShrink: 1,
    color: '#BB487C',
    fontSize: 13,
    fontWeight: '700',
    textAlign: 'left',
  },
  audioBanner: {
    position: 'absolute',
    zIndex: 11,
    minHeight: 52,
    borderRadius: 8,
    borderWidth: 2,
    paddingHorizontal: 12,
    paddingVertical: 10,
    flexDirection: 'row',
    alignItems: 'center',
    gap: 10,
    elevation: 4,
  },
  audioTextColumn: {
    flex: 1,
    gap: 2,
  },
  audioBannerTitle: {
    fontSize: 12,
    fontWeight: '800',
  },
  audioBannerText: {
    fontSize: 12,
    fontWeight: '600',
  },
  audioActionButton: {
    width: 34,
    height: 34,
    borderRadius: 8,
    borderWidth: 2,
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: 'rgba(255, 255, 255, 0.9)',
  },
  audioActionButtonPressed: {
    opacity: 0.75,
    transform: [{ scale: 0.96 }],
  },
  recenterButtonPosition: {
    position: 'absolute',
    right: 18,
    bottom: 60,
    zIndex: 1,
    gap: 10,
  },
  mapControlButton: {
    width: 44,
    height: 44,
    borderRadius: 8,
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: 'rgba(255, 255, 255, 0.92)',
    borderWidth: 2,
    borderColor: '#BB487C',
    elevation: 1,
  },
  recenterButtonWithMicPanel: {
    bottom: 220,
  },
  recenterButtonPressed: {
    opacity: 0.75,
    transform: [{ scale: 0.96 }],
  },
  micPanel: {
    position: 'absolute',
    bottom: 30,
    left: 15,
    right: 15,
    zIndex: 12,
    elevation: 12,
    borderRadius: 16,
    borderWidth: 2,
    borderColor: '#BB487C',
    padding: 16,
    gap: 12,
  },
  statusColumn: {
    flexDirection: 'column',
    alignItems: 'center',
    justifyContent: 'center',
    gap: 8,
  },
  statusRow: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
  },
  statusMic: {
    color: '#BB487C',
    fontSize: 14,
    fontWeight: '600',
  },
  roundMicButtonOuter: {
    alignSelf: 'center',
    padding: 4,
    borderRadius: 999,
    borderWidth: 6,
  },
  roundMicButton: {
    alignItems: 'center',
    justifyContent: 'center',
    alignSelf: 'center',
    width: 72,
    height: 72,
    backgroundColor: '#FFFFFF',
    borderRadius: 999,
    borderWidth: 2,
    borderColor: '#BB487C',
    elevation: 4,
  },
  roundMicButtonActive: {
    backgroundColor: '#BB487C',
    borderColor: '#FFFFFF',
  },
  confirmationText: {
    color: '#BB487C',
    fontSize: 14,
    fontWeight: '600',
    textAlign: 'center',
  },
});
